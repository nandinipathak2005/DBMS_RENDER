...
export const fetchPuzzleStage2 = async (puzzleId) => {
  const res = await fetch(`${API_BASE}/api/puzzles2/${puzzleId}`);
  if (!res.ok) throw new Error('Failed to fetch puzzle');
  return await res.json();
};

export const fetchBranch = async (branchId) => {
  const res = await fetch(`${API_BASE}/api/puzzles/branch/${branchId}`);
  if (!res.ok) throw new Error('Failed to fetch branch');
  return await res.json();
};

export const fetchBranchStage2 = async (branchId) => {
  const res = await fetch(`${API_BASE}/api/puzzles2/branch/${branchId}`);
  if (!res.ok) throw new Error('Failed to fetch branch');
  return await res.json();
};
...
export const executeQuery2 = async (query, payload) => {
  ...
  const res = await fetch(`${API_BASE}/api/query2/execute`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const updatePuzzle = async (puzzleId) => {
  // Stubbed method, ready for future implementation
  return Promise.resolve();
};
